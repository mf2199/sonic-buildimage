...
class Sfp(SfpBase):
    ...
    def __get_sfp_info(self):
        ...
        if QSFP_PORT_START <= self.port_num <= QSFP_PORT_END:
            sfp_type = QSFP_TYPE
    ...
    def __get_path_to_port_config_file(self):
        ...
        hwsku_path = (
            "/".join([platform_path, self.hwsku])
            if self._api_helper.is_host()
            else self.PMON_HWSKU_PATH
        )
    ...
    def _dom_capability_detect(self):
        ...
        if self.sfp_type == "QSFP":
            ...
            # QSFP capability byte parse, through this byte can know whether it
            # support tx_power or not.
            # TODO: in the future when decided to migrate to support SFF-8636
            #  instead of SFF-8436, need to add more code for determining the
            #  capability and version compliance in SFF-8636 dom capability
            #  definitions evolving with the versions.
            ...
            if qsfp_dom_capability_raw is not None:
                qsfp_version_compliance_raw = (
                    self.__read_eeprom_specific_bytes(
                ...
                if qsfp_version_compliance >= 0x08:
                    ...
                    self.dom_volt_supported = (
                        dom_capability["data"]["Voltage_support"]["value"]
                        == "On"
                    )
                    self.dom_rx_power_supported = (
                        dom_capability["data"]["Rx_power_support"]["value"]
                        == "On"
                    )
                    self.dom_tx_power_supported = (
                        dom_capability["data"]["Tx_power_support"]["value"]
                        == "On"
                    )
                else:
                ...
                if qsfp_option_value_raw is not None:
                    ...
                    self.dom_tx_disable_supported = (
                        optional_capability["data"]["TxDisable"]["value"]
                        == "On"
                    )
    ...
    def get_transceiver_info(self):
        ...
        if self.sfp_type == OSFP_TYPE:
            ...
            transceiver_info_dict["type"] = sfp_type_data["data"]["type"][
                "value"
            ]
            ...
            transceiver_info_dict["vendor_date"] = sfp_vendor_date_data[
                "data"
            ]["VendorDataCode(YYYY-MM-DD Lot)"]["value"]
            ...
            if self.sfp_type == QSFP_TYPE:
                ...
                for key in qsfp_compliance_code_tup:
                    if (
                        key
                        in sfp_interface_bulk_data["data"][
    ...
    def get_transceiver_threshold_info(self):
        ...
        if self.sfp_type == OSFP_TYPE:
            ...
            dom_channel_threshold_raw = self.__read_eeprom_specific_bytes(
            ...
            transceiver_dom_threshold_info_dict[
                "rxpowerhighwarning"
            ] = dom_channel_threshold_data["data"]["RxPowerHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "rxpowerlowalarm"
            ...
            transceiver_dom_threshold_info_dict[
                "txbiashighwarning"
            ] = dom_channel_threshold_data["data"]["TxBiasHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "txbiaslowalarm"
            ...
            transceiver_dom_threshold_info_dict[
                "txpowerhighwarning"
            ] = dom_channel_threshold_data["data"]["TxPowerHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "txpowerlowalarm"
            ...
            transceiver_dom_threshold_info_dict[
                "txpowerlowwarning"
            ] = dom_channel_threshold_data["data"]["TxPowerLowWarning"][
                "value"
            ]
            ...
            transceiver_dom_threshold_info_dict[
                "vcchighwarning"
            ] = dom_module_threshold_data["data"]["VoltageHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "vcclowwarning"
            ...
            transceiver_dom_threshold_info_dict[
                "txpowerhighwarning"
            ] = dom_module_threshold_data["data"]["TXPowerHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "txpowerlowwarning"
            ...
            transceiver_dom_threshold_info_dict[
                "rxpowerhighwarning"
            ] = dom_module_threshold_data["data"]["RXPowerHighWarning"][
                "value"
            ]
            transceiver_dom_threshold_info_dict[
                "rxpowerlowwarning"
    ...
    def get_rx_los(self):
        ...
        Returns:
            ...
            Note : RX LOS status is latched until a call to get_rx_los or a
            reset.
        """
        rx_los = False
        if self.sfp_type == OSFP_TYPE:
    ...
    def get_tx_fault(self):
        ...
        Returns:
            ...
            Note : TX fault status is lached until a call to get_tx_fault or a
            reset.
        """
        tx4_fault = False
        ...
        if self.sfp_type == OSFP_TYPE or not self.dom_supported:
            ...
            if dom_channel_monitor_raw is not None:
                ...
                tx4_fault = tx1_fault and tx2_fault and tx3_fault and tx4_fault
        else:
            offset = 256
            dom_channel_monitor_raw = self.__read_eeprom_specific_bytes(
                (offset + SFP_CHANNL_STATUS_OFFSET), SFP_CHANNL_STATUS_WIDTH
    ...
    def get_tx_disable_channel(self):
        ...
        Returns:
            A hex of 4 bits (bit 0 to bit 3 as channel 0 to channel 3) to
            represent TX channels which have been disabled in this SFP.
    ...
    def tx_disable(self, tx_disable):
        ...
        Args:
            tx_disable : A Boolean, True to enable tx_disable mode, False to
                         disable tx_disable mode.
        Returns:
    ...
    def tx_disable_channel(self, channel, disable):
        ...
        Args:
            channel : A hex of 4 bits (bit 0 to bit 3) which represent channel
                      0 to 3, e.g. 0x5 for channel 0 and channel 2.
            disable : A boolean, True to disable TX channels specified in
                      channel, False to enable
        Returns:
            A boolean, True if successful, False if not
        """
        if self.sfp_type == QSFP_TYPE:
            ...
            try:
                ...
                tx_disable_ctl = (
                    channel_state | tx_disable_mask[channel]
                    if disable
                    else channel_state & tx_enable_mask[channel]
                )
                buffer = create_string_buffer(1)
                buffer[0] = chr(tx_disable_ctl)
                # Write to eeprom
    ...
    def set_power_override(self, power_override, power_set):
        ...
        Returns:
            A boolean, True if power-override and power_set are set
    ...
    def get_presence(self):
        ...
        sysfs_filename = (
            "sfp_modabs" if self.sfp_type == SFP_TYPE else "qsfp_modprs"
        )
