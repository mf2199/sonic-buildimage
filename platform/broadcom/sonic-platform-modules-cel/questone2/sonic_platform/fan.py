...
PSU_FAN_MAPPING = {0: {"speed": "0x8b"}, 1: {"speed": "0x8a"}}
NULL_VAL = "N/A"
MAX_RPM_FRONT = 23000
MAX_RPM_REAR = 20500
...
class Fan(FanBase):
    ...
    def get_direction(self):
        ...
        if status:
            return (
                self.FAN_DIRECTION_INTAKE
                if "B2F" in raw
                else self.FAN_DIRECTION_EXHAUST
            )
        else:
            return NULL_VAL
    ...
    def get_speed(self):
        ...
        max_rpm = 0
        if self.is_psu_fan:
            cmd = IPMI_GET_PSU_SPEED.format(
                PSU_FAN_MAPPING[self.psu_index]["speed"]
            )
            status, raw = self._api_helper.run_command(cmd)
            if status:
    ...
    def set_speed(self, speed):
        ...
        cmd = IPMI_SET_SPEED.format(
            FAN_MAPPING[self.index]["set_speed"], speed_hex
        )
        status, raw = self._api_helper.run_command(cmd)
        if status:
            return True
